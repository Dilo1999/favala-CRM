<div>
    ...
    <form wire:submit.prevent="save" class="space-y-6">
        <div class="rounded-xl bg-gray-900 border border-gray-800 p-5 grid grid-cols-2 gap-4">
            ...
            <div></div>
            <div>
                <label class="block text-xs text-gray-400 mb-1">Invoice Date</label>
                <input type="date" wire:model="invoice_date" class="w-full rounded-lg bg-gray-800 border-gray-700 text-white text-sm" />
            </div>
            <div>
        ...
        <div class="rounded-xl bg-gray-900 border border-gray-800 p-5 overflow-x-auto">
            ...
            <table class="w-full text-sm min-w-[700px]">
                <thead><tr class="text-left text-gray-500 text-xs uppercase"><th class="pb-2">Product</th><th class="pb-2">Qty</th><th class="pb-2">Markup %</th><th class="pb-2 text-right">Amount</th><th></th></tr></thead>
                <tbody class="divide-y divide-gray-800">
                    @foreach ($items as $i => $item)
                        ...
                        <tr>
                            ...
                            <td class="py-2 pr-2 w-20"><input type="number" step="0.01" wire:model.lazy="items.{{ $i }}.qty" class="w-full rounded-lg bg-gray-800 border-gray-700 text-white text-sm" /></td>
                            <td class="py-2 pr-2 w-24"><input type="number" step="0.01" wire:model.lazy="items.{{ $i }}.markup_percent" class="w-full rounded-lg bg-gray-800 border-gray-700 text-white text-sm" /></td>
        ...
        <div class="rounded-xl bg-gray-900 border border-gray-800 p-5 max-w-sm ml-auto text-sm space-y-2">
        ...
        <div class="flex justify-end gap-2">
            <a href="{{ route('crm.invoices') }}" class="px-4 py-2 rounded-lg border border-gray-700 text-gray-300 text-sm">Cancel</a>
            <button type="submit" class="px-4 py-2 rounded-lg bg-orange-600 hover:bg-orange-500 text-white text-sm font-semibold">Create Invoice</button>
        </div>
    </form>
</div>
