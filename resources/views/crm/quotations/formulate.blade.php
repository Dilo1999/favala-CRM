    ...
    <form wire:submit.prevent="save" class="space-y-6">
        <div class="rounded-xl bg-zinc-800 border border-white/10 p-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div></div>
                <div>
                    <label class="block text-xs text-zinc-400 mb-1">Quotation Date</label>
                    <input type="date" wire:model="quotation_date" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm" />
                </div>
                <div>
                ...
                <div class="col-span-2">
                    <label class="block text-xs text-zinc-400 mb-1">Bill To (Address)</label>
                    <input type="text" wire:model="bill_to_address" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm" />
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="rounded-xl bg-zinc-800 border border-white/10 p-5 lg:col-span-2">
                ...
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <select wire:model="discount_type" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" wire:model="discount_value" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm" />
                    </div>
                    <div>
                        <label class="block text-xs text-zinc-400 mb-1">GST %</label>
                        <input type="number" step="0.01" wire:model="gst_percent" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm" />
                    </div>
                </div>
            </div>

            <div class="rounded-xl bg-zinc-800 border border-white/10 p-5 text-sm space-y-2">
                ...
                <div class="flex justify-between text-zinc-400"><span>Discount</span><span class="text-red-400">- MVR {{ number_format($this->summary['order_discount_amount'], 2) }}</span></div>
                ...
                <div class="flex justify-between text-zinc-400 pt-2"><span>Profit</span><span class="text-green-400">MVR {{ number_format($this->summary['total_profit'], 2) }} ({{ $this->summary['profit_margin'] }}%)</span></div>
            </div>
        </div>

        <div class="flex justify-end gap-2">
            <a href="{{ route('crm.quotations') }}" class="px-4 py-2 rounded-lg border border-white/10 text-zinc-300 text-sm">Cancel</a>
            <button type="submit" class="px-4 py-2 rounded-lg bg-accent hover:bg-accent-hover text-white text-sm font-semibold">{{ $recordId ? 'Save Changes' : 'Create Quotation' }}</button>
        </div>
    </form>
</div>
