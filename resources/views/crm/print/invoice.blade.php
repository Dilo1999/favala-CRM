...
<body class="p-10 max-w-3xl mx-auto text-sm">
    ...
    <div class="grid grid-cols-2 gap-6 mb-8">
        <div>
            <p class="text-xs text-gray-500 uppercase font-semibold mb-1">Billed To</p>
            <p class="font-medium">{{ $record->bill_to_name ?? $record->customer?->company_name }}</p>
            <p class="text-gray-600">{{ $record->bill_to_phone }}</p>
            <p class="text-gray-600">{{ $record->customer?->tin }}</p>
        </div>
        <div class="text-right">
            <p class="text-xs text-gray-500 uppercase font-semibold mb-1">Payment Status</p>
            <p class="font-medium">{{ ucfirst($record->payment_status) }}</p>
            <p class="text-gray-600">Balance Due: MVR {{ number_format($record->balance_due, 2) }}</p>
        </div>
    </div>
        ...
        <tbody>
            @foreach ($record->items as $i => $item)
                <tr class="border-b border-gray-200">
                    ...
                    <td class="py-2 text-right">MVR {{ number_format($item->rate, 2) }}</td>
                    <td class="py-2 text-right">MVR {{ number_format($item->amount, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="flex justify-end mb-8">
        <div class="w-64 space-y-1">
            ...
            <div class="flex justify-between font-bold text-base border-t border-gray-800 pt-1"><span>Invoice Total</span><span>MVR {{ number_format($record->grand_total, 2) }}</span></div>
            <div class="flex justify-between"><span class="text-gray-600">Total Paid</span><span>MVR {{ number_format($record->amount_paid, 2) }}</span></div>
    ...
    <p class="text-xs text-gray-400 border-t border-gray-200 pt-4">Thank you for your business.</p>
</body>
</html>
