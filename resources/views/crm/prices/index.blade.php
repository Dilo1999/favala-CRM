        ...
        <div>
            ...
            @if ($filteredProduct)
                <p class="text-sm text-gray-400 mt-1">
                    ...
                    <button wire:click="$set('productFilter', null)" class="text-orange-400 ml-2">Clear</button>
                </p>
            @endif
        </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-800">
                @forelse ($prices as $price)
                    <tr class="hover:bg-gray-800/40 cursor-pointer" wire:click="edit({{ $price->id }})">
                        <td class="p-3 text-gray-400">{{ $price->created_at->format('d M Y') }}</td>
                        <td class="p-3 text-white">
                            {{ $price->product?->description }}
                            <span class="text-gray-500 text-xs block">{{ $price->product?->code }}</span>
                        </td>
                        <td class="p-3 text-gray-300">{{ $price->vendor?->company_name }}</td>
                        <td class="p-3 text-white font-medium">MVR {{ number_format($price->price, 2) }}</td>
                        <td class="p-3 text-gray-400">{{ $price->addedBy?->name }}</td>
                        <td class="p-3 text-right" wire:click.stop>
                            <x-row-menu>
                                <button wire:click="edit({{ $price->id }})" class="block w-full text-left px-3 py-1.5 text-gray-200 hover:bg-gray-700">Edit</button>
                                <button wire:click="delete({{ $price->id }})" wire:confirm="Delete this price entry?" class="block w-full text-left px-3 py-1.5 text-red-400 hover:bg-gray-700">Delete</button>
                            </x-row-menu>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="p-6 text-center text-gray-500">No price entries found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-md p-6">
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    <div>
                        ...
                        <select wire:model="form.product_id" class="w-full rounded-lg bg-gray-800 border-gray-700 text-white text-sm">
                    ...
                    <div>
                        ...
                        <select wire:model="form.vendor_id" class="w-full rounded-lg bg-gray-800 border-gray-700 text-white text-sm">
                            <option value="">Select…</option>
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Price (without GST)</label>
                        <input type="number" step="0.01" wire:model="form.price" class="w-full rounded-lg bg-gray-800 border-gray-700 text-white text-sm" />
                    ...
                    <div class="flex justify-end gap-2 mt-2">
                        ...
                        <button type="submit" class="px-4 py-2 rounded-lg bg-orange-600 hover:bg-orange-500 text-white text-sm font-semibold">Save Price</button>
                    </div>
                </form>
            </div>
        </div>
