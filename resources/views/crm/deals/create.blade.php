    ...
    <form wire:submit.prevent="save" class="max-w-3xl space-y-6">
        <div class="rounded-xl bg-zinc-800 border border-white/10 p-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs text-zinc-400 mb-1">Deal Date</label>
                    <input type="date" wire:model="deal_date" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm" />
                </div>
                <div>
                    <label class="block text-xs text-zinc-400 mb-1">Request Source</label>
                ...
                <div>
                    <label class="block text-xs text-zinc-400 mb-1">Assigned Staff</label>
                ...
                <div>
                    <label class="block text-xs text-zinc-400 mb-1">Deal Stage</label>
                ...
                <div class="col-span-2">
                    <label class="block text-xs text-zinc-400 mb-1">Additional Details</label>
        ...
        <div class="rounded-xl bg-zinc-800 border border-white/10 p-5">
            ...
            <div class="space-y-3">
                @foreach ($products as $i => $row)
                    <div class="flex gap-3 items-start">
                        ...
                        <div class="w-28">
                            <input type="number" step="0.01" wire:model="products.{{ $i }}.qty" placeholder="Qty" class="w-full rounded-lg bg-zinc-700 border-white/10 text-white text-sm" />
                        </div>
                        @if (count($products) > 1)
            ...
            <button type="button" wire:click="addProductRow" class="mt-3 text-sm text-accent font-medium">+ Add product</button>
        </div>

        <div class="flex justify-end gap-2">
            <a href="{{ route('crm.deals') }}" class="px-4 py-2 rounded-lg border border-white/10 text-zinc-300 text-sm">Cancel</a>
            <button type="submit" class="px-4 py-2 rounded-lg bg-accent hover:bg-accent-hover text-white text-sm font-semibold">Create Deal</button>
        </div>
    </form>
</div>
